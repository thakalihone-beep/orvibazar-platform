...
@props([
    'image' => 'https://via.placeholder.com/300x300/1a1a1a/ffffff?text=Product',
    'title' => 'Product Title',
    'price' => 0,
    'original_price' => null,
    'rating' => 0,
...
@php
    ...
    $cardId = 'product-' . ($product_id ?? uniqid());
@endphp
...
<div id="{{ $cardId }}"
    ...
    <div class="product-image-container" style="position: relative; {{ $layout === 'horizontal' ? 'width: 40%; flex-shrink: 0;' : 'padding-top: 100%;' }} overflow: hidden; background: var(--color-off-white);">
        ...
        @if($sale && $stock !== 'out-of-stock')
            <span class="sale-badge" style="position: absolute; top: var(--spacing-sm); right: var(--spacing-sm); background: var(--color-sale-bg); color: white; padding: 4px 12px; border-radius: var(--radius-full); font-size: var(--font-size-xs); font-weight: var(--font-weight-bold); text-transform: uppercase; z-index: 2; animation: pulse-badge 2s infinite;">
                <i class="fas fa-tag" style="margin-right: 4px;"></i> SALE
            </span>
        @endif

        <!-- Discount Percentage Badge -->
        @if($discount > 0 && $stock !== 'out-of-stock')
            <span class="discount-badge" style="position: absolute; top: var(--spacing-sm); left: var(--spacing-sm); background: var(--color-primary); color: var(--color-text-light); padding: 4px 10px; border-radius: var(--radius-full); font-size: var(--font-size-xs); font-weight: var(--font-weight-bold); z-index: 2;">
                -{{ $discount }}%
            </span>
        @endif
        ...
        @if($show_stock)
            <span class="stock-badge {{ $stock }}"
                  style="position: absolute; bottom: var(--spacing-sm); left: var(--spacing-sm); padding: 4px 12px; border-radius: var(--radius-full); font-size: var(--font-size-xs); font-weight: var(--font-weight-medium); z-index: 2; background: {{ $stockClasses[$stock] ?? 'var(--color-mid-gray)' }}; color: white; display: flex; align-items: center; gap: 4px;">
                <i class="fas {{ $stockIcons[$stock] ?? 'fa-info-circle' }}"></i>
                {{ $stockLabels[$stock] ?? 'Unknown' }}
            </span>
        @endif

        <!-- Quick Add to Cart Button -->
        @if($show_quick_add && $stock !== 'out-of-stock')
            <button class="quick-add-btn"
                    onclick="quickAddToCart('{{ $cardId }}', '{{ $title }}', {{ $price }})"
    ...
    <div class="product-info" style="padding: var(--spacing-md); {{ $layout === 'horizontal' ? 'flex: 1; display: flex; flex-direction: column; justify-content: center;' : '' }}">
        ...
        <div class="product-price" style="font-size: var(--font-size-xl); font-weight: var(--font-weight-bold); color: var(--color-primary); display: flex; align-items: center; gap: var(--spacing-sm); flex-wrap: wrap; margin-top: var(--spacing-xs);">
            <span class="sale-price" style="color: {{ $discount > 0 ? 'var(--color-sale)' : 'var(--color-primary)' }};">
            ...
            @if($discount > 0)
                ...
                <span style="font-size: var(--font-size-xs); color: var(--color-success); font-weight: var(--font-weight-medium);">
                    Save ${{ number_format($original_price - $price, 2) }}
                </span>
            @endif
        </div>
        ...
        @if($layout === 'horizontal' && $stock !== 'out-of-stock')
            <div style="display: flex; gap: var(--spacing-sm); margin-top: var(--spacing-md);">
                <button onclick="addToCart('{{ $cardId }}', '{{ $title }}', {{ $price }})"
        ...
        @if($layout === 'vertical' && $stock !== 'out-of-stock' && !$show_quick_add)
            <button onclick="addToCart('{{ $cardId }}', '{{ $title }}', {{ $price }})"
